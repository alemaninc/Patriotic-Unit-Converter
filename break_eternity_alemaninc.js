// dependent on 'alemaninc_utility.js', load before this
"use strict";
(function (global, factory) {
	...
	var f_lambertw = function(z, tol = 1e-10) {
		...
		throw Util.error("Iteration failed to converge: " + z);
		//return Number.NaN;
	};
	...
	function () {
		...
		Decimal.prototype.add = function (value) {
			...
			throw Util.error("Bad arguments to add: " + this + ", " + value);
		};
		...
		Decimal.prototype.mul = function (value) {
			...
			throw Util.error("Bad arguments to mul: " + this + ", " + value);
		};
		...
		Decimal.prototype.lambertw = function() {
			...
			{
				throw Util.error("lambertw is unimplemented for results less than -1, sorry!");
			}
			else if (this.mag < 0)
			{
...
*/
		...
		Decimal.prototype.add1Pow = function(p) { // (x + 1) ^ p - 1 that works for low values
			if (p.eq(1)) {return this.add(1)}
			if (this.mul(p).abs().lt(1e-10)) {return this.mul(p).add(1)}
			if (this.abs().lt(1e-10)) {return this.mul(p).exp()}
			return this.add(1).pow(p)
		}

		Decimal.prototype.add1PowSub1 = function(p) { // (x + 1) ^ p - 1 that works for low values
			if (p.eq(1)) {return this}
			if (this.add1Log().mul(p).lt(1e-10)) {return this.add1Log().mul(p)}
			if (this.lt(1e-10)) {return this.mul(p).exp().sub(1)}
...
const constant = Util.deepFreeze({
...
Decimal.notations = {
	...
	list:{
		"Alemaninc Ordinal":function(x){
			...
			if (output.gte(constant.d24)) {
				return "ω<sub>10 000 000 000</sub>"
			}
			...
			return ["α","β","γ","δ","ε","ζ","η","θ","ι","κ","λ","μ","ν","ξ","ο","π","ρ","σ","τ","υ","φ","χ","ψ","ω"][output.floor().toNumber()]+"<sub>"+Decimal.notations.formatSmall(number.mul(precision).floor().div(precision).toNumber())+"</sub>";
		},
		"BE Default":function(x,sub=undefined,p=3){
		...
		"Engineering":function(x,sub="Engineering",p=2){
			if (x.gte("eeeee6")) {
				return Decimal.notations.list["Hyper-E"](x,sub)
			}
			let leadingEs = Decimal.notations.leadingEs(x)
			if (leadingEs===0) {
				x=x.mul(1.0000001);
				return x.log10().mod(constant.d3).pow10().toPrecision(p+1)+"e"+Decimal.notations.formatSmall(x.log10().div(constant.d3).floor().mul(constant.d3).toNumber())
			}
			return Array(leadingEs+1).join("e")+Decimal.notations.list["Engineering"](x.layerplus(-leadingEs),sub,p+1)
		},
		"Hyperand":function(x,sub="Hyperand",p=3) {
			if (x.gte("eeeee6")) {
				return "A² "+Decimal.notations.list["Hyperand"](x.quad_slog(c.d10),sub,p+1)
			}
			let leadingEs = Decimal.notations.leadingEs(x)+1
			return ((leadingEs===0)?"":(leadingEs===1)?"A ":(leadingEs+"A "))+Decimal.notations.formatSmall(x.layerplus(-leadingEs).toNumber(),p)
		},
		"Hyper-E":function(x,sub="Hyper-E",p=3){
			let height = Math.floor(x.slog().toNumber())
			if (height>1e6) {
				return "E#"+Decimal.notations.list[sub](N(height),sub)
			}
			return "E"+((x.mag>=1e10)?Math.log10(Math.log10(x.mag)):Math.log10(x.mag)).toFixed(p)+"#"+Decimal.notations.formatSmall(height)
		},
		"Infinity":function(x){
			if (x.gte("eeeee6")) {
				return (Math.log2(x.quad_slog(constant.d10).toNumber())/1024).toFixed(6)+"Ω"
			}
			let infinities = 0
			while (x.gte(c.e6)) {
		...
		"Logarithm":function(x,sub="Logarithm",p=3) {
			...
			if (leadingEs===0) {
				x=x.mul(1.0000001);
				return "e"+Decimal.notations.formatSmall(x.log10().toNumber(),p)
			}
			return Array(leadingEs+1).join("e")+Decimal.notations.list["Logarithm"](x.layerplus(-leadingEs),sub,p+1)
		},
		"Mixed scientific":function(x,sub="Mixed scientific",p=2){
			...
			let leadingEs = Decimal.notations.leadingEs(x)
			if (leadingEs===0) {
				return Decimal.notations.list[x.gte(constant.e33)?"Scientific":"Standard"](x,sub,p)
			}
			return Array(leadingEs+1).join("e")+Decimal.notations.list["Mixed scientific"](x.layerplus(-leadingEs),sub,p+1)
		},
		"Scientific":function(x,sub="Scientific",p=2){
			if (x.gte("eeeee6")) return Decimal.notations.list["Hyper-E"](x,sub);
			let leadingEs = Decimal.notations.leadingEs(x);
			let effp = p
			if (x.layerplus(-leadingEs).gte(constant.ee4)) {
				effp--;
			}
			if (x.layerplus(-leadingEs).gte(constant.ee5)) {
				effp--;
			}
			if (leadingEs===0) {
				x=x.mul(1.0000001);
				return x.log10().mod(constant.d1).pow10().mul(10**effp).floor().div(10**effp).toFixed(effp)+"e"+Decimal.notations.formatSmall(x.log10().floor().toNumber())
			}
		...
		"Standard":function(x,sub="Standard",p=2){
			...
			if (leadingEs===0) {
				if (x.lt(constant.e33)) {
					return x.log10().mod(constant.d3).pow10().toPrecision(p+1)+" "+["M","B","T","Qa","Qt","Sx","Sp","Oc","No"][Math.floor(x.log10().toNumber()/3-2)];
				}
				let height = x.log10().div(constant.d3).sub(constant.d1).floor().toNumber()
				let out = []
				let hp = p+2
				let sequence = Decimal.notations.standard.sequence(Math.floor(Math.log10(height)),hp)
				for (let i=10;i>=0;i--) {
					out.push(Decimal.notations.standard.str(height,i,sequence[i]))
				}
				...
				if (height>=10**hp) {
					...
					out = x.log10().mod(constant.d3).pow10().toPrecision(p+1)+" "+out
				}
				return out
			}
			return Array(leadingEs+1).join("e")+Decimal.notations.list["Standard"](x.layerplus(-leadingEs),sub,p+1)
		},
		"Tetration":function(x,sub="Tetration"){
			let height = x.quad_slog(c.d10)
			if (height<1e6) {
				return "10 ⇈ "+height.toPrecision(7)
			}
			return "10 ⇈ "+Decimal.notations.list[sub](height,sub,6)
		},
		"Time":function(x){
...
Decimal.format = function(value,precision=0,notation="Scientific",subnotation=notation,highPrecision=0) {
	...
	if (x.lt(constant.em5)) return "(1 ÷ "+Decimal.format(x.recip(),precision,notation,subnotation,highPrecision)+")";
	if (x.lt(constant.e6)) return Decimal.notations.formatSmall(x.toNumber(),precision)
